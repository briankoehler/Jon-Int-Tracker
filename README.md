...
If tracking many summoners, you will likely need to adjust the time interval of the ```get_int()``` task located inside ```bot/bot.py```.  This is a simple task, and can be done by opening ```bot.py``` and finding the following on line 46:
```
@tasks.loop(seconds=20)
```
Simply change the 20 to a higher number (such as 30) and you will have no issues.  You can also reduce the number if desired.
# Planned Features
